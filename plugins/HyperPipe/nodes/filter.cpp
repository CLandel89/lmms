...
struct HPFilterModel : public HPModel::Node {
	HPFilterModel(Instrument* instrument) :
			Node(instrument),
			m_reso(make_shared<FloatModel>(0.5f, BasicFilters<>::minQ(), 10.0f, 0.01f, instrument, QString("Q/Resonance"))),
			m_amp(make_shared<FloatModel>(10.0f, 0.0f, 40.0f, 0.1f, instrument, QString("filter makeup")))
	{}
	shared_ptr<FloatModel> m_reso;
...
public:
	HPFilterView(HPView* view) :
			m_reso(view, "Q/Resonance"),
			m_amp(view, "filter makeup")
	{
	...
	void setModel(shared_ptr<HPModel::Node> model) {
		shared_ptr<HPFilterModel> modelCast = static_pointer_cast<HPFilterModel>(model);
		m_reso.setModel(modelCast->m_reso.get());
...
template<> shared_ptr<HPFilterModel> Definition::newNodeImpl() {
	return make_shared<HPFilterModel>(m_instrument);
}
